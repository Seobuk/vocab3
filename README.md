...
- **카드 학습** — 뜻과 예문은 가려져 있어 먼저 떠올린 뒤 톡으로 확인(단어 톡 = 읽기, 뜻 칸 톡 = 뜻 보이기/가리기, 예문은 한 번 톡 = 읽기 · 두 번 톡 = 해석). 위로 스와이프 = 외웠다, 아래로 = 아직, 좌우 = 이전/다음 카드. 되돌리기, 학습 순서 랜덤.
- **한→영 출력 모드** — 우리말만 보고 영어 단어와 예문을 말해 보는 훈련.
- **영어 문장 공부 (v2.12)** — 졸업한 단어의 예문(v2.17부터 유튜브에서 꾹 눌러 담은 문장도)을 한글로 먼저 보여 주고, 탭하면 가려 둔 영어가 보이며 읽어 줘요. 위로 밀면 쉬움·아래로 밀면 어려움, 왼쪽으로 밀면 고르지 않고 다음 문장·오른쪽으로 밀면 이전 문장(v2.33) — 어려운 문장일수록 가중치가 쌓여 더 자주 나오고, 멈출 때까지 계속 이어져요.
...
- **유튜브 쉐도잉 (Gemini)** — 홈의 "유튜브"에서 영상 링크를 넣으면 Gemini가 영상을 듣고 영어 문장을 시간순으로 정리합니다(한글 해석은 가려 두고 누르면 보임, 익힐 표현은 밑줄). 문장을 누르면 앱 안 유튜브 플레이어가 그 시점부터 재생하고 "문장마다 멈춤"으로 한 문장씩 따라 말하기(쉐도잉), 재생한 문장의 단어를 누르면 뜻이 나오고 1단계·대기 단어장에 바로 추가. 공개 영상만 되며, 재생은 YouTube API 서비스(YouTube 약관·Google 개인정보처리방침 적용)를 씁니다. v2.3: 문장은 끝까지 한 문장으로(화면 자막 줄바꿈 무시) 정리하고 문장 끝 시간에서 멈춰요(다음 문장까지 안 읽음). v2.33: 영어 문장 공부도 옆으로 밀어 넘겨요(왼쪽 = 다음 문장, 오른쪽 = 이전 문장, 고르지 않고). v2.32: 처음 설치하면 화면마다 처음 들어갈 때 기능 안내(가리키며 설명)가 한 번 나오고, 화면 속 긴 사용법 문구는 뺐어요(설정 → 사용법 → "기능 안내 다시 보기"). v2.31: 학습 카드의 단어 뜻은 한 번 톡으로 열고 닫아요. v2.30: 앱 전체 톡 규칙 통일(한 번 톡 = 읽기·재생, 두 번 톡 = 한글) — 학습 카드 스피커 대신 단어·예문을 톡, 유튜브는 단어를 누르면 그 단어부터 재생, 문장 공부에 처음 보는 문장·유튜브 문장 제안을 섞음. v2.29: 드라이브 불러오기 확인 창이 떠 있는 동안 새 폰의 빈 기록이 드라이브 원본을 덮을 수 있던 문제를 고쳤어요(확인한 뒤에만 연동), 앱을 오갈 때 쓸데없이 올리지 않게. v2.28: Google 드라이브 연동 — 설정에서 드라이브 파일 하나를 연결해 두면 학습 기록을 자동 저장하고, 새 폰에선 "드라이브에서 불러오기"로 그대로 이어져요(로그인 없이, Gemini 키는 제외). v2.27: 앱 자체 업데이트 — 앱을 열면 GitHub 에서 새 버전을 확인해 "업데이트"를 누르면 받아서 바로 설치해요(처음 한 번 "이 출처 허용", 설정 → 앱 업데이트에서 끄기·지금 확인). v2.26: 폴드 안쪽처럼 넓은 화면에선 꾹 누르기·단어 창이 오른쪽 아래에 작게 뜨고, 학습 카드가 가로로 넓어졌어요(긴 예문이 잘리지 않게). 유튜브 문장도 한 번 톡 = 재생, 두 번 톡 = 한글 보기. v2.25: 단어장에 정렬(많이 틀린 순·적게 틀린 순·랜덤), 졸업 탭에 문장 공부에 담은 유튜브 문장도, 오른쪽에 틀림·맞음·문장 공부 기록·마지막으로 본 때. 예문은 한 번 톡 = 읽기, 두 번 톡 = 가려 둔 한글 보이기/가리기. v2.24: 소리만 받은 영상(m4a)에서 이어 받은 문장 시간이 몇 초 당겨지거나 앞부분이 빠질 수 있던 것, 마지막 부분을 끝까지 안 받고 멈출 수 있던 것 등 v2.23 리뷰에서 찾은 문제를 고쳤어요. v2.23: 폰에 받아 둔 긴 영상은 "이어서 정리하기"가 그 소리를 10분씩 잘라 보내 조각마다 바로 붙어요(몇 분째·지금 하는 일·취소 표시, 정리하는 동안 화면 켜 둠). 폴드를 접고 펴거나 앱을 종료했다 다시 열 때 남은 옛 화면이 손으로 고친 시간·합친 문장을 덮어쓰던 문제를 고쳤어요(설정 → 데이터 → "켤 때 상태로"). 버튼·카드·아이콘을 더 둥글게. v2.22: 가로 화면 "문장 시간 수정"을 새로 — 위 막대 시계 버튼을 누르면 아래에 구간 띠(받은 영상은 소리 파형)가 뜨고 [ ] 손잡이를 끌거나 [시작|끝]을 골라 ±·"지금"으로 고쳐요. 앱 전체 디자인 정리(그라데이션·과한 그림자·여러 강조색·이모지 아이콘 정리, 한글 줄바꿈). v2.21: 받은 영상의 왼쪽·오른쪽을 두 번 톡 누르면 앞·뒤 문장. 긴 영상 "이어서 정리하기"는 뒷부분만 보내 훨씬 빨라졌고, 정리 중엔 몇 분째인지 보여요. 영상 끝에 영어 말이 없으면(음악·한국어) 한 번 확인한 뒤 "이어서 정리하기"를 더 권하지 않아요. v2.20: "이어서 정리하기"는 목록 맨 위(영상 길이를 알면)와 맨 아래(늘)에. v2.19: 정리가 영상 끝보다 일찍 멈추면 자동으로 이어 받고, 예전에 일찍 멈춘 영상은 "이어서 정리하기"로 뒷부분만 받아 붙여요. 꾹 누르기 선택창에 "📖 단어 뜻 보기". v2.18: 단어는 두 번 톡 눌러야 뜻(한 번 톡은 그 문장 다시 재생), 1시간짜리 같은 긴 영상도 끝까지 정리(답이 잘리면 이어서 받기, 붐빔·오류는 자동 재시도). v2.17: 문장을 꾹 누르면 선택창 — 📚 영어 문장 공부에 넣기(졸업 단어 예문과 함께 나옴, 가중치 똑같이), 📋 복사, ⤒⤓ 앞·뒤 문장과 합치기, ✂ 쪼개기(나눌 단어를 누름), 바로 되돌리기. v2.16: 화면에 박힌 자막을 따라 한 문장이 여러 줄로 쪼개지던 것을 . ? ! 로 끝날 때까지 되붙여요(이미 정리한 영상도 앱을 열 때 한 번, 다시 정리하지 않고). v2.15: 받은 영상은 화면을 탭하면 멈추고 다시 탭하면 이어서 재생(표시 없이), 제목줄의 📌로 영상을 맨 위에 고정하면 목록을 올려도 영상이 가려지지 않아요(세로 화면). v2.14: 링크를 넣으면 폰이 360p 영상을 직접 받아 앱 안에만 저장해 두고(안드로이드 13 이상), 유튜브 플레이어 대신 앱 플레이어로 재생해요 — 유튜브 화면 요소가 없고 오프라인에서도 쉐도잉. 받은 소리의 파형으로 문장 시작·끝을 실제 조용한 곳에 자동으로 맞춰요(손으로 고친 시간이 우선). 예전 영상은 "⬇ 오프라인 저장". v2.11: 가로 화면에서 영상 아래 "✎ 문장 시간 수정"으로 누른 문장의 시작·끝을 0.1·0.5초씩 또는 "지금"(영상 위치)으로 고쳐 저장(고친 문장은 ✎ 표시, 그 자리에서 정확히 재생·멈춤), 문장을 꾹 누르면 영어 문장 복사, 멈출 때 영상 위에 뜨는 제목줄·"동영상 더보기"를 안 보이게. v2.9: 문장 시간을 더 정확하게 — Gemini 가 조금 생각하고(생각 모드) 영상을 1초에 2장씩 보며 정리해요(예전 영상은 "다시 정리하기"). v2.8: 새로 정리할 땐 "Yes."·"Right." 같은 아주 짧은 문장(3단어 이하)을 가까운 앞뒤 문장에 합쳐요. v2.7: 문장 끝에서 정확히 멈추고, 재생 중엔 영상 위 조작 버튼이 뜨지 않아요. v2.6: 영상 화면에서 폰을 가로로 돌리면 왼쪽 영상 · 오른쪽 스크립트. v2.5: 1분 넘는 곳부터 문장 시간이 어긋나던 문제 수정(예전 영상은 "다시 정리하기"). v2.4: 영상은 맨 위 제자리에 있고, 목록을 올리면 문장이 영상 위를 덮어 화면을 넓게 써요(따라다니는 창 없음). "한 번 더"·"문장마다" 버튼은 오른쪽 아래(오른손 엄지). 예전에 정리한 영상은 "다시 정리하기"로 새 규칙 적용.
